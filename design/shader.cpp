...
void Shader::newShaderProgramFromFile(const char* vshFile,
                                      const char* fshFile){
    this->prog = new ShaderProgram();
    if( ! prog->vertexShader(vshFile))
        return;
    if (! prog->fragmentShader(fshFile))
        return;
    
    glBindAttribLocation(prog->progId,0, "position");
    glBindAttribLocation(prog->progId,1, "norm");
    glBindAttribLocation(prog->progId,2, "color");
...
void Shader::newShaderProgramFromFile(const char* vshFile,
    ...
    if (! prog->fragmentShader(fshFile))
        return;
    if( ! prog->geometryShader(fshFile))
        return;
...
void Shader::getAllUniforms(){
    uniformLocations = map<std::string, GLuint>();
    GLuint unicount;
    ...
    glGetObjectParameterivA(prog->progId, GL_OBJECT_ACTIVE_ATTRIBUTES,
                            &unicount);
    for( Gluint i = 0; i < unicount; i++){
        glGetActiveUniform (prog->shaderID, i, buffSize, &bytesWritten, &size,
                            &unif_type, buff);
        Gluint loc = prog->UniformLocation(buff);
        uniformLocations[std::string(buff)] = loc;
    }
